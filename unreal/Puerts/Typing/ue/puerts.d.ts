...
declare module "ue" {
    ...
    namespace uclass 
    {
        ...
        class ClassKey { 
            /**
             * assign a list of value to the meta data
             * @param InValue 
             * @param InRemains 
             */
            assign(InValue: string, ...InRemains: string[]): ClassKey;
        }
         ...
         */
        const NotBlueprintType: ClassKey;
         ...
         */
        const NotBlueprintable: ClassKey;
         ...
         */
        const DefaultToInstanced: ClassKey;
         ...
         *      Class can be constructed from editinline New button.
         */
        const editinlinenew: ClassKey;
        /**
         * @brief
         *      Class can't be constructed from editinline New button.
         */
        const noteditinlinenew: ClassKey;
        /**
         * @brief
         *      Class not shown in editor drop down for class selection.
         */
        const hidedropdown: ClassKey;
         ...
         */
        const AdvancedClassDisplay: ClassKey;
        ...
        {
            /**
             * assign the meta data value
             * @param InValue 
             */
            assign(InValue: string): MetaKey;
        }
         ...
         */
        const DeprecationMessage: MetaKey;
         ...
         */
        const RestrictedToClasses: MetaKey;
         ...
         */
        const ShowWorldContextPin: MetaKey;
         ...
         */
        const DontUseGenericSpawnObject: MetaKey;

        /**
         * @brief
         *      [ClassMetadata] Expose a proxy object of this class in Async Task node.
         */
        const ExposedAsyncProxy: MetaKey;
         ...
         */
        const BlueprintThreadSafe: MetaKey;
    ...
    namespace ufunction
    {
        ...
        class FunctionKey
        {
             ...
             * @param InRemains 
             */
            assign(InValue: string, ...InRemains: string[]): FunctionKey;
        }
         ...
         */
		const BlueprintImplementableEvent: FunctionKey;
         ...
         */
		const BlueprintNativeEvent: FunctionKey;
         ...
         */
		const SealedEvent: FunctionKey;

        /**
         * @brief 
		 *      This function is executable from the command line.
         */
		const Exec: FunctionKey;
         ...
         */
		const InternalUseParam: FunctionKey;
         ...
         */
        const BlueprintInternalUseOnly: MetaKey;
         ...
         */
		const ArrayTypeDependentParams: MetaKey;
         ...
         */
		const DeprecationMessage: MetaKey; //, (Commented out so as to avoid duplicate name with version in the Class section, but still show in the function section)
         ...
         */
		const ExpandEnumAsExecs: MetaKey;

        /**
         * @brief
		 *      Synonym for ExpandEnumAsExecs
         */
		const ExpandBoolAsExecs: MetaKey;

        /**
         * @brief
		 *      [ClassMetadata] [PropertyMetadata] [FunctionMetadata] The name to display for this class, property, or function instead of auto-generating it from the name.
         */
		const DisplayName: MetaKey; //, (Commented out so as to avoid duplicate name with version in the Class section, but still show in the function section)

        /**
         * @brief
         *      [ClassMetadata] [PropertyMetadata] [FunctionMetadata] The name to use for this class, property, or function when exporting it to a scripting language. May include deprecated names as additional semi-colon separated entries.
         */
        const ScriptName: MetaKey; // (Commented out so as to avoid duplicate name with version in the Class section, but still show in the function section)
         ...
         */
		const ScriptNoExport: MetaKey; //, (Commented out so as to avoid duplicate name with version in the Property section, but still show in the function section)
         ...
         */
		const ScriptMethodSelfReturn: MetaKey;
         ...
         */
		const UnsafeDuringActorConstruction: MetaKey;
         ...
         */
		const BlueprintAutocast: MetaKey;
         ...
         */
		const NotBlueprintThreadSafe: MetaKey;
         ...
         */
		const DeterminesOutputType: MetaKey;
         ...
         */
		const DynamicOutputParam: MetaKey;
         ...
         */
		const AnimBlueprintFunction: MetaKey;
    ...
    namespace uproperty 
    {
        ...
        class PropertyKey
        {
             ...
             * @param InRemains 
             */
            assign(InValue: string, ...InRemains: string[]): PropertyKey;
        }
         ...
         */
		const AssetRegistrySearchable: PropertyKey;
         ...
         */
		const SaveGame: PropertyKey;

        /**
         * @brief
		 *      MC Delegates only.  Property should be exposed for calling in blueprint code
         */
		const BlueprintCallable: PropertyKey;

        /**
         * @brief
		 *      MC Delegates only. This delegate accepts (only in blueprint) only events with BlueprintAuthorityOnly.
         */
		const BlueprintAuthorityOnly: PropertyKey;

        /**
         * @brief
		 *      Property shouldn't be exported to text format (e.g. copy/paste)
         */
		const TextExportTransient: PropertyKey;

        /**
         * @brief
		 *      Property shouldn't be serialized, can still be exported to text
         */
		const SkipSerialization: PropertyKey;

        /**
         * @brief
		 *      If true, the self pin should not be shown or connectable regardless of purity, const, etc. similar to InternalUseParam
         */
		const HideSelfPin: PropertyKey;
         ...
         */
		const AllowPreserveRatio: MetaKey;
         ...
         */
		const AllowPrivateAccess: MetaKey;
         ...
         */
		const BlueprintBaseOnly: MetaKey;
         ...
         */
		const ContentDir: MetaKey;

        /**
         * @brief
		 *      [PropertyMetadata] This property is deprecated, any blueprint references to it cause a compilation warning.
         */
		const DeprecatedProperty: MetaKey;

        /**
         * @brief
		 *      [ClassMetadata] [PropertyMetadata] [FunctionMetadata] Used in conjunction with DeprecatedNode, DeprecatedProperty, or DeprecatedFunction to customize the warning message displayed to the user.
         */
		const DeprecationMessage: MetaKey; //, (Commented out so as to avoid duplicate name with version in the Class section, but still show in the property section)

        /**
         * @brief
		 *      [ClassMetadata] [PropertyMetadata] [FunctionMetadata] The name to display for this class, property, or function instead of auto-generating it from the name.
         */
		const DisplayName: MetaKey; // (Commented out so as to avoid duplicate name with version in the Class section, but still show in the property section)

        /**
         * @brief
		 *      [ClassMetadata] [PropertyMetadata] [FunctionMetadata] The name to use for this class, property, or function when exporting it to a scripting language. May include deprecated names as additional semi-colon separated entries.
         */
		const ScriptName: MetaKey; //, (Commented out so as to avoid duplicate name with version in the Class section, but still show in the property section)
         ...
         */
		const RelativeToGameDir: MetaKey;
         ...
         */
		const IgnoreForMemberInitializationTest: MetaKey;
         ...
         */
		const MakeStructureDefaultValue: MetaKey;
         ...
         */
		const RelativeToGameContentDir: MetaKey;

        /**
         * @brief
		 *      [PropertyMetadata] [FunctionMetadata] Flag set on a property or function to prevent it being exported to a scripting language.
         */
		const ScriptNoExport: MetaKey;
		 ...
		 *      [FunctionMetadata] [InterfaceMetadata] Metadata that identifies an integral property as a bitmask.
         */
		const Bitmask: MetaKey; //, for property

        /**
         * @brief
         *      [FunctionMetadata] [InterfaceMetadata] Metadata that associates a bitmask property with a bitflag enum.
         */
		const BitmaskEnum: MetaKey; //, for property
