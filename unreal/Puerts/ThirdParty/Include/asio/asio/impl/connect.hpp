...
namespace detail
{
  struct default_connect_condition
  {
    ...
    bool operator()(const puerts_asio::error_code&, const Endpoint&)
    {
      return true;
    }
  };
  ...
  inline typename Protocol::endpoint deref_connect_result(
      Iterator iter, puerts_asio::error_code& ec)
  {
  ...
  inline Iterator call_connect_condition(ConnectCondition& connect_condition,
      const puerts_asio::error_code& ec, Iterator next, Iterator end,
    ...
    puerts_asio::error_code& ec)
{
  ...
  if (!ec)
    ec = puerts_asio::error::not_found;
...
namespace detail
{
  ...
  protected:
    ...
    void check_condition(const puerts_asio::error_code& ec,
  ...
  protected:
    ...
    template <typename Iterator>
    void check_condition(const puerts_asio::error_code&, Iterator&, Iterator&)
    {
    }
  };
  ...
  public:
    ...
    void operator()(puerts_asio::error_code ec, int start = 0)
    {
  ...
  //private:
    template <typename Iterator>
    void process(puerts_asio::error_code ec,
      ...
      switch (start_ = start)
      {
        ...
        for (;;)
        {
          ...
          if (start)
          {
            ec = puerts_asio::error::not_found;
            ASIO_HANDLER_LOCATION((__FILE__, __LINE__, "async_connect"));
            puerts_asio::post(socket_.get_executor(),
                detail::bind_handler(
        ...
        ASIO_MOVE_OR_LVALUE(RangeConnectHandler)(handler_)(
            static_cast<const puerts_asio::error_code&>(ec),
        ...
        const ConnectCondition& connect_condition) const
    {
      ...
      range_connect_op<Protocol, Executor, EndpointSequence, ConnectCondition,
        typename decay<RangeConnectHandler>::type>(socket_, endpoints,
          connect_condition, handler2.value)(puerts_asio::error_code(), 1);
    }
