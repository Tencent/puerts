...
struct prefer_only_property<InnerProperty,
    typename void_type<
      decltype(puerts_asio::declval<const InnerProperty>().value())
    >::type>
{
  InnerProperty property;
  ...
  ASIO_CONSTEXPR auto value() const
    ASIO_NOEXCEPT_IF((
      noexcept(puerts_asio::declval<const InnerProperty>().value())))
    -> decltype(puerts_asio::declval<const InnerProperty>().value())
  {
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((
      is_nothrow_prefer<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return puerts_asio::prefer(ex, p.property);
  }
  ...
  && !defined(__clang__) // Clang crashes if noexcept is used here.
    ASIO_NOEXCEPT_IF((
      is_nothrow_query<const Executor&, const InnerProperty&>::value))
       ...
       //   && !defined(__clang__)
  {
    return puerts_asio::query(ex, p.property);
  }
};
