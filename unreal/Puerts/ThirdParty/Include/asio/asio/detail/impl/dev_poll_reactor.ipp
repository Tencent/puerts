...
dev_poll_reactor::dev_poll_reactor(puerts_asio::execution_context& ctx)
  : puerts_asio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
{
  puerts_asio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, puerts_asio::error::operation_aborted);
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, puerts_asio::error::operation_aborted);
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      puerts_asio::error_code ec = puerts_asio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    puerts_asio::error_code ec(errno,
        puerts_asio::error::get_system_category());
    puerts_asio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
