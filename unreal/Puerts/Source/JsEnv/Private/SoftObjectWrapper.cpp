...
v8::Local<v8::FunctionTemplate> FSoftObjectWrapper::ToFunctionTemplate(v8::Isolate* Isolate)
{
	...
	Result->PrototypeTemplate()->Set(FV8Utils::ToV8String(Isolate, "LoadSynchronous"),
		v8::FunctionTemplate::New(Isolate, &LoadSynchronous));
	Result->PrototypeTemplate()->Set(FV8Utils::ToV8String(Isolate, "Get"),
...
static void GenericObjectGet(const v8::FunctionCallbackInfo<v8::Value>& Info, FSoftObjectPtrObjectGetter Getter)
{
	...
	if (Obj)
	{
		...
		Info.GetReturnValue().Set(FV8Utils::IsolateData<IObjectMapper>(Isolate)->FindOrAdd(Isolate, Context,
			Obj->GetClass(), Obj));
	}
}
...
{
	GenericObjectGet(Info, [](FSoftObjectPtr* Ptr)
	{
