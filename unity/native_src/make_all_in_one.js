...
async function runMake() {
    ...
    const OUTPUT_PATH = pwd + '/../Assets/Plugins/' + getoutputPluginPath(options, BuildConfig);
...
function getoutputPluginPath(options, BuildConfig) {
    ...
    if (backend.includes("v8")) {
        backEndFolderName = `V8_${config}`;
    } else if (backend.includes("quickjs")) {
        backEndFolderName = `Quickjs`;
