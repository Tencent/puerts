...
PESAPI_EXTERN pesapi_value pesapi_native_object_to_value_js(pesapi_env env, const void* type_id, void* object_ptr, int call_finalize);
...
PESAPI_EXTERN int pesapi_has_caught_js(pesapi_scope scope);
PESAPI_EXTERN const char* pesapi_get_exception_as_string_js(pesapi_scope scope, int with_stack);
...
PESAPI_EXTERN int pesapi_set_property_js(pesapi_env env, pesapi_value object, const char* key, pesapi_value value);
PESAPI_EXTERN int pesapi_get_private_js(pesapi_env env, pesapi_value object, void** out_ptr);
PESAPI_EXTERN int pesapi_set_private_js(pesapi_env env, pesapi_value object, void* ptr);
...
PESAPI_EXTERN int pesapi_set_property_uint32_js(pesapi_env env, pesapi_value object, uint32_t key, pesapi_value value);
...
PESAPI_EXTERN void pesapi_set_registry_js(pesapi_env env, pesapi_registry registry);
PESAPI_EXTERN void TranToString(pesapi_value val);
...
int JS_ToBool(int *pres, JSValue val)
{
...
static int JS_ToInt32(int32_t *pres, JSValue val)
{
    ...
    case JS_TAG_STRING:
        {
            /* convert UTF8 string to int32 */
            if (val.u.str.ptr && val.u.str.len > 0) {
                ret = (int32_t)atoi(val.u.str.ptr);
            } else {
                ret = 0;
            }
        }
    ...
    case JS_TAG_STRING16:
        {
            /* convert UTF16 string to int32 */
            if (val.u.str.ptr && val.u.str.len > 0) {
                /* simple conversion: extract ASCII characters and convert */
                std::string utf8_str;
                utf8_str.reserve(val.u.str.len);
                const uint16_t* str16 = (const uint16_t*)val.u.str.ptr;
                for (uint32_t i = 0; i < val.u.str.len; i++) {
                    if (str16[i] < 128) {
                        utf8_str.push_back((char)str16[i]);
                    } else {
                        /* non-ASCII character, stop conversion */
                        break;
                    }
                }
                ret = (int32_t)atoi(utf8_str.c_str());
            } else {
                ret = 0;
            }
        }
...
static inline int JS_IsNumber(JSValue val)
{
    uint32_t tag = JS_VALUE_GET_TAG(val);
...
{
    return pesapi_get_value_generic<int>(env, pvalue, JS_ToBool);
}
...
const uint16_t* pesapi_get_value_string_utf16(pesapi_env env, pesapi_value pvalue, uint16_t* buf, size_t* bufsize)
{
    ...
    if (bufsize)
    {
        if (JS_TAG_STRING16 != JS_VALUE_GET_TAG(jsvalue))
        {
            TranToString(pvalue);
            jsvalue = *(reinterpret_cast<JSValue*>(pvalue));
        }
        if (JS_TAG_STRING16 != JS_VALUE_GET_TAG(jsvalue))
        {
            return buf;
...
{
    return pesapi_is_generic(env, pvalue, [](JSValue val) -> int {
...
int pesapi_set_property_uint32(pesapi_env env, pesapi_value pobject, uint32_t key, pesapi_value pvalue)
{
    return pesapi_set_property_uint32_js(env, pobject, key, pvalue);
}
...
extern "C"
{
    struct pesapi_ffi* InitPapiNative()
    {
        ...
        api->create_binary_by_value = &pesapi::webglimpl::pesapi_create_binary;
        ...
        api->set_registry = &pesapi_set_registry_js;
