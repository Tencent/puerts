...
class FunctionCallbackInfoPtrManager {
    ...
    GetMockPointer(args) {
        ...
        for (var i = 0; i < args.length; i++) {
            ...
            this.engine.unityApi.HEAP32[(buffer >> 2) + (2 + i * this.argumentValueLengthIn32)] = GetType(this.engine, args[i]); // jsvaluetype
            this.engine.unityApi.HEAP32[(buffer >> 2) + (3 + i * this.argumentValueLengthIn32)] = (0, getFromJSArgument_1.$GetArgumentFinalValue)(this.engine, args[i], ((buffer >> 2) + (4 + i * this.argumentValueLengthIn32)) << 2); // value
        }
        return buffer;
    }
...
function $GetArgumentFinalValue(engine, val, lengthOffset) {
    const type = (0, library_1.GetType)(engine, val);
    switch (type) {
        case 4: return +val;
